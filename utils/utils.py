...
def readlines_from_txt(XY,data_folder='/home/comp/chongyin/DataSets/Liver-NASH/'):
    all_XY=[]
    for item in XY:
        f=open(item,'r')
        lines_=f.readlines()
        lines=[line.strip().replace('/home/comp/chongyin/DataSets/Liver-NASH/',data_folder) for line in lines_]
        all_XY+=lines
        f.close()
    return all_XY


def balance_sampler(x,ratio=1.0):
    # x: list
    dictD={}
    for item in x:
        ss=item.split(' ')
        if ss[1] not in dictD:
            dictD[ss[1]]=[ss[0]]
        else:
            dictD[ss[1]].append(ss[0])

    newX=[]
    for key, value in dictD.items():
        temp=[f'{xx} {key}' for xx in value[:int(len(value)*ratio)]]
        newX+=temp
    random.seed(1024)
    random.shuffle(newX)
    return newX
